 ...
 */
void XrossBoard::InitializeXrossBoard(wxString& xb, wxDir& xbDir) 
{
...
void XrossBoard::DeleteBAllDatFile(wxCommandEvent& event) {
     ...
     if (result == wxID_YES) 
     {
	  ...
	  wxString dirname = XrossBoardUtil::AssembleFilePath(empty, empty, KIND_BOARD__DAT);
	  wxDir targetDir(dirname);
	  
	  wxArrayString  files;
	  size_t count = XrossBoardUtil::GetFileNamesRecursive(targetDir, files);
	  // ファイル削除
