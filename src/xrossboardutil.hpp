...
#ifndef XROSSBOARDUTIL_HPP_
#define XROSSBOARDUTIL_HPP_
...
public:
     ...
     static std::string UrlEncode(const wxString& str) {
	  return XrossBoardUtil::UrlEncode(std::string(str.mb_str()));
     };
     /**
      * 文字列中の実体参照文字を変換する
      */
      ...
      */
     template <class T> 
     static void SetXrossBoardProperties(const wxString& key, const T& value);
     /**
      * プロパティファイルの指定されたKEYを読み取る
      */
     template <class T> 
     static void GetXrossBoardProperties(const wxString& key, T* value);
     /**
      * プロパティファイルの指定されたKEYがあるかどうか確認する
      */
     static bool GetXrossBoardEntryExist(const wxString& key);
     /**
      * プロパティファイルの指定されたKEYを削除する
      */
     static void DeleteXrossBoardPropertyEntry(const wxString& key);
     /**
